<div class="my-7 mx-7">
    ...
    <form wire:submit.prevent="save">
        ...
        <div class="mb-6">
          ...
          <input wire:model="confirm_password" type="password" id="repeat-password" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light">
        </div>
        <div class="text-red-500">
            @error('confirm_password') {{ $message }} @enderror
        </div>
        @if (session('error'))
        <span class="text-success lead">{{ session('error') }}</span>
    @endif
